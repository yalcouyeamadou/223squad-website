...
function Services() {
  return (
    <div className="px-6 py-16 max-w-5xl mx-auto">
      <h1 className="text-4xl font-extrabold text-center mb-2 text-stone-900 dark:text-white">
        Nos <span className="text-amber-500">Services</span>
      </h1>
      <p className="text-stone-600 dark:text-stone-400 text-center mb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {services.map((service) => (
          <div
            key={service.title}
            className="bg-stone-100 dark:bg-stone-900 border border-amber-800/30 dark:border-amber-800/40 rounded-xl p-6 hover:border-amber-500/60 transition-colors"
          >
            <h2 className="text-xl font-bold text-amber-500 mb-2">
              {service.title}
            </h2>
            <p className="text-stone-700 dark:text-stone-300">{service.desc}</p>
          </div>
        ))}
      </div>
    </div>
